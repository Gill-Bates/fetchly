...
// Every cancel test above fires hidden.bs.modal directly, which is what real
// Bootstrap does *after* something dismissed the dialog. What makes the close
// icon and the cancel button dismiss it in the first place is the
// data-bs-dismiss attribute - confirm.js wires no click handler on either, so
// dropping the attribute would leave two dead buttons that no promise-level
// assertion in this file could catch.
test("close and cancel are wired to Bootstrap's dismiss, not to a handler", async () => {
    const pending = confirmModal({ message: "Go?", cancelText: "Not now" });
    const { root, closeBtn, cancelBtn, acceptBtn } = modalParts();

    assert.equal(closeBtn.getAttribute("data-bs-dismiss"), "modal");
    assert.equal(cancelBtn.getAttribute("data-bs-dismiss"), "modal");
    assert.equal(cancelBtn.textContent, "Not now");
    // The confirm button must NOT be a dismiss target: it resolves true from
    // its own handler and hides the modal itself. data-bs-dismiss there would
    // race the hidden.bs.modal listener that resolves false.
    assert.equal(acceptBtn.getAttribute("data-bs-dismiss"), null);
...
test("falls back to a native prompt when Bootstrap is unavailable", async () => {
    ...
    // Restored rather than reassigned to a fresh stub: leaving a different
    // window.confirm behind than the one installFakeDom() set makes this
    // suite's cleanup depend on being the last test in the file.
    const savedConfirm = globalThis.window.confirm;
    ...
    try {
        ...
        globalThis.window.confirm = savedConfirm;
    }
});
